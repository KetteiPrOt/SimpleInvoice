...
class Signer extends Controller
{
    private $issuer = 'CN=AUTORIDAD DE CERTIFICACION SUBCA-2 SECURITY DATA,OU=ENTIDAD DE CERTIFICACION DE INFORMACION,O=SECURITY DATA S.A. 2,C=EC';

    private $path = '/opt/lampp/htdocs/SimpleInvoice/storage/app';
    ...
    {
        $user = Auth::user();
        Storage::disk('local')->put('certificate.p12', $user->certificate);
        $this->password = $user->certificate_password;
        ...
        $signed_invoice = Storage::disk('local')->get('invoice.xml');
        $signed_invoice = str_replace('</factura>', $XAdES_BES, $signed_invoice);

        foreach($this->generatedFiles as $generatedFile){
            Storage::disk('local')->delete($generatedFile);
        }
    ...
    private function extractFiles(string $password): void
    {
        ...
        $this->execute(
            "openssl1.0 pkcs12 -in $this->path/certificate.p12 "
        ...
        $this->execute(
            "openssl1.0 x509 -in $this->path/certificate.pem "
            . "-outform DER -out $this->path/certificate.der"
        );
        $this->execute(
            "openssl1.0 sha1 -out $this->path/certificate.sha1 "
            . "-binary $this->path/certificate.der"
        );
        $this->execute(
            "openssl1.0 base64 -e -in $this->path/certificate.sha1 "
            . "-out $this->path/certificate_hash.txt"
        );
        // Extract modulus and exponent (modulus_exponent.txt)
        $this->execute(
            "openssl1.0 rsa -in $this->path/certificate.pem "
    ...
    private function extractData(): void
    {
        // Extract Serial
        $output = $this->execute(
            "openssl1.0 x509 -noout -serial -in $this->path/certificate.pem"
        ...
        $invoice = Storage::disk('local')->get('invoice.xml');
        $invoice_for_hash = str_replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n", '', $invoice);
        Storage::disk('local')->put('invoice_for_hash.xml', $invoice_for_hash);
        $this->execute(
            "openssl1.0 sha1 -out $this->path/invoice_hash.sha1 "
            . "-binary $this->path/invoice_for_hash.xml"
        );
        $this->execute(
            "openssl1.0 base64 -e -in $this->path/invoice_hash.sha1 "
            . "-out $this->path/invoice_hash.txt"
        );
        $this->invoice_hash = str_replace(
            "\n", '',
            Storage::disk('local')->get('invoice_hash.txt')
        );

        // Obtain SHA1 hash from Certificate X509
        $this->certificate_hash = str_replace(
            "\n", '',
            Storage::disk('local')->get('certificate_hash.txt')
        );

        // Obtain Certificate X509 in Base64
        $certificate = Storage::disk('local')->get('certificate.pem');
        ...
        // Extract Modulus
        $modulus_exponent = Storage::disk('local')->get('modulus_exponent.txt');
    ...
    {
        Storage::disk('local')->put("file_for_hash.$extension", $string);

        $this->execute(
            "openssl1.0 sha1 -out $this->path/file_for_hash.sha1 "
            . "-binary $this->path/file_for_hash.$extension"
        ); // clean file_for_hash.$extension ; file_for_hash.sha1
        $this->execute(
            "openssl1.0 base64 -e -in $this->path/file_for_hash.sha1 "
            . "-out $this->path/file_hash.txt"
        ); // clean file_hash.txt
        $hash = str_replace(
            "\n", '',
            Storage::disk('local')->get('file_hash.txt')
        );
        Storage::disk('local')->delete(["file_for_hash.$extension", 'file_for_hash.sha1', 'file_hash.txt']);
        return $hash;
    }
    ...
    {
        Storage::disk('local')->put('SignedInfo.txt', $SignedInfo);

        // Generate SHA1 signature file
        $this->execute(
            "openssl1.0 dgst -sha1 -sign $this->path/private_key.pem -passin pass:$this->password -out $this->path/signature.sha1 $this->path/SignedInfo.txt"
        );

        // Generate base64 signature file
        $this->execute(
            "openssl1.0 base64 -in $this->path/signature.sha1 -out $this->path/signature.txt"
        );

        return Storage::disk('local')->get('signature.txt');
    }
}
